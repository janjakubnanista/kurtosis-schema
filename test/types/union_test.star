...
def test_union_type_transform(plan):
    ...
    expect.eq(main.union_type(main.int_type()).transform(_transform_multiply_by_two).parse(1), 2)
...
def test_union_type_pipe(plan):
    ...
    expect.eq(main.union_type(main.int_type()).pipe(int_multiplied).pipe(int_greeted).parse(1), "hey 2")